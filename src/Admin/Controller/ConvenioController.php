...
class ConvenioController extends Controller
{
   ...
   public function __construct()
   {
       ...
       $this->hoConvenioModel = new Model\ConvenioModel();
       $this->valSegPerfils = new Model\SegUsuariosPerfilModel();
       $this->apps = $this->pathApps(__DIR__);
       $this->entidad = $this->hoConvenioModel->tabla;
       $this->vista = $this->pathVista();
       $this->comps = $this->apps .' - '. $this->entidad .' - '. $this->vista;
   }

    /**
    * Mostrar el index de la vistaConvenio
    * @param: GET $resquest
    */ 
   public function runConvenioIndex($request)
   {
     ...
     $this->tpl->renders('view::vistas/convenio/'.$this->pathVista().'/index');
   }

    /**
    * Listar registros de Convenio
    * @param: POST $resquest
    * @return: XML $result
    */ 
   public function runConvenioListar($request)
   {
      ...
      $rows = $this->hoConvenioModel->getConvenioListar($request->getParameter(),$result);
    ...
    /**
    * Crear registros de Convenio
    * @param: POST $resquest
    * @return: JSON $result
    */ 
   public function runConvenioCreate($request)
   {
      ...
      parent::runValidarMascarasVista('vistas/convenio/', $this->pathVista(),$request->postParameter());

      $result = $this->hoConvenioModel->setConvenioCreate($request->postParameter());
      if(is_null($result)){
        $dataJson['error']='1';
    ...
    /**
    * Ver registros de Convenio
    * @param: POST $resquest
    * @return: JSON $result
    */ 
   public function runConvenioShow($request)
   {
      // Verificar permisologia
      $this->permisos = 'CONSULTA|CONTROL TOTAL';
      $this->validatePermisos($this->valSegPerfils->valSegPerfilRelacionUser($this->comps,$this->permisos),true);

      $result = $this->hoConvenioModel->getConvenioShow($request->postParameter());
      $this->json($result);
   }

    /**
    * Eliminar registros de Convenio
    * @param: POST $resquest
    * @return: JSON $result
    */ 
   public function runConvenioDelete($request)
   {
      // Verificar permisologia
      $this->permisos = 'BAJA|CONTROL TOTAL';
      $this->validatePermisos($this->valSegPerfils->valSegPerfilRelacionUser($this->comps,$this->permisos),true);

      $result = $this->hoConvenioModel->remConvenioDelete($request->postParameter());
      if(is_null($result)){
        $dataJson['error']='0';
    ...
    /**
    * Actualizar registros de Convenio
    * @param: POST $resquest
    * @return: JSON $result
    */ 
   public function runConvenioUpdate($request)
   {
      // Verificar permisologia
      ...
      $result = $this->hoConvenioModel->setConvenioUpdate($request->postParameter());
      if(is_null($result)){
        $dataJson['error']='0';
