...
class GabyController extends Controller
{
   use Security;
   public $model;
   public $session;
   ...
   public function runGabyIndex($request)
   {
     ...
     $this->tpl->renders('view::vistas/gaby/'.$this->pathVista().'/index');
   }

    /**
    * Listar registros de Gaby
    * @param: POST $resquest
    * @return: XML $result
    */ 
   public function runGabyListar($request)
   {
      ...
      $rows = $this->hoGabyModel->getGabyListar($request->getParameter(),$result);
    ...
    /**
    * Crear registros de Gaby
    * @param: POST $resquest
    * @return: JSON $result
    */ 
   public function runGabyCreate($request)
   {
      ...
      parent::runValidarMascarasVista('vistas/gaby/', $this->pathVista(),$request->postParameter());

      $result = $this->hoGabyModel->setGabyCreate($request->postParameter());
      if(is_null($result)){
        $dataJson['error']='1';
    ...
    /**
    * Ver registros de Gaby
    * @param: POST $resquest
    * @return: JSON $result
    */ 
   public function runGabyShow($request)
   {
      // Verificar permisologia
      $this->permisos = 'CONSULTA|CONTROL TOTAL';
      $this->validatePermisos($this->valSegPerfils->valSegPerfilRelacionUser($this->comps,$this->permisos),true);

      $result = $this->hoGabyModel->getGabyShow($request->postParameter());
      $this->json($result);
   }
    ...
    * @return: JSON $result
    */ 
   public function runGabyDelete($request)
   {
      // Verificar permisologia
      $this->permisos = 'BAJA|CONTROL TOTAL';
      $this->validatePermisos($this->valSegPerfils->valSegPerfilRelacionUser($this->comps,$this->permisos),true);

      $result = $this->hoGabyModel->remGabyDelete($request->postParameter());
      if(is_null($result)){
        $dataJson['error']='0';
    ...
    * @return: JSON $result
    */ 
   public function runGabyUpdate($request)
   {
      // Verificar permisologia
      ...
      $result = $this->hoGabyModel->setGabyUpdate($request->postParameter());
      if(is_null($result)){
        $dataJson['error']='0';
