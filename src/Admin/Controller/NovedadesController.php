...
class NovedadesController extends Controller
{
   ...
   public function __construct()
   {
       ...
       $this->hoNovedadesModel = new Model\NovedadesModel();
       $this->valSegPerfils = new Model\SegUsuariosPerfilModel();
       $this->apps = $this->pathApps(__DIR__);
       $this->entidad = $this->hoNovedadesModel->tabla;
       $this->vista = $this->pathVista();
       $this->comps = $this->apps .' - '. $this->entidad .' - '. $this->vista;
   }

    /**
    * Mostrar el index de la vistaNovedades
    * @param: GET $resquest
    */ 
   public function runNovedadesIndex($request)
   {
     ...
     $this->tpl->renders('view::vistas/novedades/'.$this->pathVista().'/index');
   }

    /**
    * Listar registros de Novedades
    * @param: POST $resquest
    * @return: XML $result
    */ 
   public function runNovedadesListar($request)
   {
      ...
      $rows = $this->hoNovedadesModel->getNovedadesListar($request->getParameter(),$result);
    ...
    * @return: JSON $result
    */ 
   public function runNovedadesCreate($request)
   {
      ...
      parent::runValidarMascarasVista('vistas/novedades/', $this->pathVista(),$request->postParameter());

      $result = $this->hoNovedadesModel->setNovedadesCreate($request->postParameter());
      if(is_null($result)){
        $dataJson['error']='1';
    ...
    /**
    * Ver registros de Novedades
    * @param: POST $resquest
    * @return: JSON $result
    */ 
   public function runNovedadesShow($request)
   {
      // Verificar permisologia
      $this->permisos = 'CONSULTA|CONTROL TOTAL';
      $this->validatePermisos($this->valSegPerfils->valSegPerfilRelacionUser($this->comps,$this->permisos),true);

      $result = $this->hoNovedadesModel->getNovedadesShow($request->postParameter());
      $this->json($result);
   }
    ...
    * @return: JSON $result
    */ 
   public function runNovedadesDelete($request)
   {
      // Verificar permisologia
      $this->permisos = 'BAJA|CONTROL TOTAL';
      $this->validatePermisos($this->valSegPerfils->valSegPerfilRelacionUser($this->comps,$this->permisos),true);

      $result = $this->hoNovedadesModel->remNovedadesDelete($request->postParameter());
      if(is_null($result)){
        $dataJson['error']='0';
    ...
    /**
    * Actualizar registros de Novedades
    * @param: POST $resquest
    * @return: JSON $result
    */ 
   public function runNovedadesUpdate($request)
   {
      // Verificar permisologia
      ...
      $result = $this->hoNovedadesModel->setNovedadesUpdate($request->postParameter());
      if(is_null($result)){
        $dataJson['error']='0';
