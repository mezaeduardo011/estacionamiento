...
Config.Box3 = {
    ...
    listadoUniversoTablas: function () {
        ...
        Config.currentRequest =  $.ajax({
                    url: '/getEntidadesSeleccionadas',
                    type: "POST",
                    headers: {
                        'X-Auth-Token' : $('#csrf_token').val()
                    },
                    data: {'conn':conexion,'entidad':table},
                    dataType: 'JSON',
    ...
    /**
     * Esto es una función permite gestionar las vistas que se van a crear o actualizar
     * @memberof Config.Box3.getConfiguracionVista
     */
    getConfiguracionVista: function (connect, tabla, id, select) {
        ...
        Config.currentRequest = $.ajax({
                    url: '/getConfiguracionVista',
                    type: "POST",
                    headers: {
                        'X-Auth-Token' : $('#csrf_token').val()
                    },
                    data: {'connect':connect, 'tabla':tabla , 'vista':id},
                    dataType: 'JSON',
                    success : function(dataJson) {
                        ...
                        $.get('/getListApp', function (dataJson) {
                        ...
                        setTimeout(function () {
                            // Permite selecccionar la aplicacion que ya esta cargada
                            $('#apps').val(dataJson[0].apps)
                            // Permite Seleccionar los item que tienen mascaras cargados
                            $.each(mascaras, function(key, value){
                                $('select#mascara_'+key).val(value);
                            })
                        }, 1000);
            ...
            if($(this).val()!=0) {
                ...
                if(apps.val()==0){
                    ...
                    $.ajax({
                        ...
                        data: {'apps':apps.val(),'conexionId':conex,'tipo':activo.val()},
                        dataType: 'JSON',
                        success : function(dataJson) {
                            var rowss = [];
                            ...
                            activo.parent('td').next('td').children('select').html(opt).change(function (event) {
                                ...
                                $.ajax({
                                    url: '/getVistasColumns',
                                    type: "POST",
                                    headers: {
                                        'X-Auth-Token' : $('#csrf_token').val()
                                    },
                                    data: {'apps': apps.val(), 'conexionId': conex, 'entidad': v.val()},
                                    dataType: 'JSON',
                                    success : function(dataJson) {
                                        Config.html = '';
